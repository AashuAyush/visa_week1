...
public class StreamExample {
	...
	public static void main(String[] args) {
		...
		products.stream().filter(p -> p.getCategory().equals("mobile")).forEach(p -> System.out.println(p)); //stream of products; toString of product is called

		System.out.println("Mobile Names");
		products.stream().filter(p -> p.getCategory().equals("mobile")).map(p-> p.getName()).forEach(name -> System.out.println(name)); //stream of strings after map
	
		products.stream().filter(p -> p.getCategory().equals("mobile")).map(p-> p.getName()).forEach(System.out::println);  //method reference
